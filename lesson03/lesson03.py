...
def lasso_path_coefs(Z, y, alphas):
    ...
    """
    return np.array([Lasso(alpha=a).fit(Z, y).coef_ for a in alphas])
...
def ridge_path_coefs(Z, y, alphas):
    ...
    """
    return np.array([Ridge(alpha=a).fit(Z, y).coef_ for a in alphas])
...
def fit_logreg_numeric(parts, numeric):
    ...
    model.fit(Xtr[numeric], ytr)
    return {
        "model": model,
        "val_auc": roc_auc_score(yva, model.predict_proba(Xva[numeric])[:, 1]),
        "test_auc": roc_auc_score(yte, model.predict_proba(Xte[numeric])[:, 1]),
...
def fit_logreg_ohe(parts, numeric, categorical):
    ...
    (Xtr, ytr, _), (Xva, yva, _), (Xte, yte, _) = (
        parts["train"], parts["val"], parts["test"])
    model = Pipeline([("prep", build_ohe_pipeline(numeric, categorical)),
                      ("clf", LogisticRegression(max_iter=1000))])
    model.fit(Xtr, ytr)
    return {
        "model": model,
        "val_auc": roc_auc_score(yva, model.predict_proba(Xva)[:, 1]),
        "test_auc": roc_auc_score(yte, model.predict_proba(Xte)[:, 1]),
        "n_features": int(model["prep"].transform(Xtr[:1]).shape[1]),
    }
...
def tree_depth_sweep(Ztr, ytr, Zva, yva, depths, min_samples_leaf=1):
    ...
    for d in depths:
        t = DecisionTreeClassifier(max_depth=d,
                                   ...
                                   random_state=RANDOM_SEED).fit(Ztr, ytr)
        train_aucs.append(roc_auc_score(ytr, t.predict_proba(Ztr)[:, 1]))
        val_aucs.append(roc_auc_score(yva, t.predict_proba(Zva)[:, 1]))
...
def fit_tree(parts, numeric, categorical, max_depth=6, min_samples_leaf=50):
    ...
    (Xtr, ytr, _), (Xva, yva, _), (Xte, yte, _) = (
        parts["train"], parts["val"], parts["test"])
    model = Pipeline([
        ("prep", build_ohe_pipeline(numeric, categorical)),
...
def boost_by_hand(x, y, n_rounds=3):
    ...
    """
    X = x.reshape(-1, 1)
    pred = np.full(len(y), y.mean(), dtype=float)
    out = {"f0": float(y.mean()), "stumps": [],
           "train_pred": [pred.copy()],
    ...
    for _ in range(n_rounds):
        ...
        out["stumps"].append(stump)
        out["train_pred"].append(pred.copy())
        out["train_mse"].append(float(np.mean((y - pred) ** 2)))
    return out
...
def fit_catboost_default(parts, categorical):
    ...
    (Xtr, ytr, _), (Xva, yva, _), (Xte, yte, _) = (
        parts["train"], parts["val"], parts["test"])
    model = CatBoostClassifier(iterations=500, random_seed=RANDOM_SEED,
                               verbose=0, cat_features=categorical,
...
def fit_catboost_eval_auc(parts, categorical, params):
    ...
    """
    (Xtr, ytr, _), (Xva, yva, _) = parts["train"], parts["val"]
    model = CatBoostClassifier(iterations=800, random_seed=RANDOM_SEED,
                               verbose=0, cat_features=categorical,
                               allow_writing_files=False, eval_metric="AUC",
                               **params)
    model.fit(Xtr, ytr, eval_set=(Xva, yva), early_stopping_rounds=50,
              use_best_model=True)
...
def make_optuna_objective(parts, categorical):
    ...
    def objective(trial):
        ...
        _, val_auc = fit_catboost_eval_auc(parts, categorical, params)
        return val_auc
    return objective
