...
def standardize(X, mean=None, std=None):
    ...
    if std is None:
        std = X.std(axis=0)
    # A constant column has std 0 and would give inf. Guard it.
    safe_std = np.where(std == 0, 1.0, std)
    return (X - mean) / safe_std, mean, safe_std
...
def percentile_manual(values, q):
    ...
    s = np.sort(values)
    pos = (q / 100.0) * (s.size - 1)          # fractional index
    lo = int(np.floor(pos))
    hi = int(np.ceil(pos))
    frac = pos - lo
...
def rank_average(values):
    ...
    # Group equal values and overwrite each group with its mean rank.
...
def group_mean(labels, values):
    ...
    keys, codes = np.unique(labels, return_inverse=True)
    totals = np.bincount(codes, weights=values, minlength=keys.size)
    counts = np.bincount(codes, minlength=keys.size)
    return keys, totals / counts
...
def crosstab_rate(labels_a, labels_b, values):
    ...
    keys_a, code_a = np.unique(labels_a, return_inverse=True)
    keys_b, code_b = np.unique(labels_b, return_inverse=True)
    flat = code_a * keys_b.size + code_b
    n_cells = keys_a.size * keys_b.size
    totals = np.bincount(flat, weights=values, minlength=n_cells)
    counts = np.bincount(flat, minlength=n_cells)
    return keys_a, keys_b, (totals / np.maximum(counts, 1)).reshape(keys_a.size, -1)
...
def apply_log_transform(X, cols):
    ...
    out = X.copy()                       # .copy() -- or we mutate the caller's array
...
def one_hot(labels):
    ...
    keys, codes = np.unique(labels, return_inverse=True)
    return np.eye(keys.size)[codes], keys
...
def add_cross_features(X):
    ...
    n, d = X.shape
    # (n, d, 1) * (n, 1, d) broadcasts to (n, d, d): every pair, every row.
    outer = X[:, :, None] * X[:, None, :]
    i, j = np.triu_indices(d)            # upper triangle incl. diagonal
...
def stratified_split(y, fractions=(0.6, 0.2, 0.2), seed=0):
    ...
    assignment = np.empty(y.size, dtype=int)

    for cls in np.unique(y):
        idx = np.flatnonzero(y == cls)
        idx = gen.permutation(idx)                 # shuffle within the class
        cuts = (np.cumsum(fractions)[:-1] * idx.size).astype(int)
        for part, chunk in enumerate(np.split(idx, cuts)):
...
def fit_normal_equation(X, y):
    ...
    X1 = np.hstack([np.ones((X.shape[0], 1)), X])     # intercept column
...
def fit_gradient_descent(X, y, lr=0.1, n_iters=2000):
    ...
    for it in range(n_iters):
        resid = X @ w + b - y                 # (n,)
        history[it] = np.mean(resid ** 2)
        w -= lr * (2.0 / n) * (X.T @ resid)   # (d,)
        b -= lr * (2.0 / n) * resid.sum()
    return w, b, history
...
def confusion_matrix_numpy(y_true, y_pred):
    ...
    codes = 2 * y_true.astype(int) + y_pred.astype(int)
    return np.bincount(codes, minlength=4).reshape(2, 2)
...
def roc_curve_numpy(y_true, score):
    ...
    # Tied scores must be committed to as a block: keep only the last index of
    # each run of equal scores.
    keep = np.r_[np.flatnonzero(np.diff(score[order])), y_true.size - 1]
